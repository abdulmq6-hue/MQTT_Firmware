...
function initKioskSidebar() {
    ...
    // Block nav clicks when sidebar is collapsed — first tap only expands
    document.querySelectorAll('.nav-item[data-view]').forEach(item => {
        item.addEventListener('click', (e) => {
            if (!sidebar.classList.contains('expanded') && window.innerWidth <= 1280) {
                // Sidebar is collapsed: block navigation, just expand
                e.stopImmediatePropagation();
                e.preventDefault();
                sidebar.classList.add('expanded');
                return;
            }
            // Sidebar is expanded: allow navigation, then collapse
            setTimeout(() => sidebar.classList.remove('expanded'), 400);
        }, true); // capture phase — runs BEFORE the nav handler in setupEventListeners
    });
